...
class RateService
{
    ...
    const FLCLShippingServiceURL = "http://api-int.flavorcloud.com/api/Rates";

    // Create the rate request and make the service call
    public function getRate()
    {
        $client = CurlClient::instance();
        $requestParams = $this->_buildRateRequestParams();
        $response = $client->request("POST", self::FLCLShippingServiceURL, array('Content-Type:application/json'), $requestParams);
        return $response[0];
    }


    // build out the json rate request
    private function _buildRateRequestParams()
    {
        try
        {
            $params = array();
